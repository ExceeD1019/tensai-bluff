...
const sample = TopicSchema.parse(
  JSON.parse(
    readFileSync(fileURLToPath(new URL("../topics/sample-topic.json", import.meta.url)), "utf8"),
  ),
);
...
describe("同梱サンプルお題", () => {
  it("スキーマに準拠", () => {
    expect(() => TopicSchema.parse(sample)).not.toThrow();
  });

  it("構造チェックにエラーが無い", () => {
    const errs = checkStructure(sample).filter((i) => i.severity === "error");
    expect(errs).toEqual([]);
  });
});
...
describe("checkStructure()", () => {
  it("階層配分が崩れると TIER_COUNT", () => {
    ...
    t.facts[1]!.tier = "specific"; // surface 0 個
